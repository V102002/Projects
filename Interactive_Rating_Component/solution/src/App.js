...
function App() {
  ...
  return (
    <div className="App">
      ...
      {isSubmitted && <ThankYou items={items} setIsSubmitted_again={setIsSubmitted} />}
    </div>
  );
}

const ThankYou = ({ items, setIsSubmitted_again }) => {
  return (
    <div className="wrapper">
      <img src={thanks} alt="" className="block mx-auto"/>
      <div className="flex items-center justify-center ">
        <p className="p-rating text-center mt-5 bg-gray-700 rounded-full text-sm px-4 pt-1">You selected {items} out of 5</p>
      </div>
      ...
      <div className="text-center">
        <button onClick={() => {setIsSubmitted_again(false)}} className="btn-rating w-full text-white uppercase tracking-wide pt-3 pb-2 rounded-full">Rate Again</button>
      </div>
    </div>
  )
}
