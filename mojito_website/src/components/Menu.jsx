...
const Menu = () => {
  ...
  return (
    <section id='menu' aria-labelledby='menu-heading'> {/*The user can swap between different section within this menu */}
      ...
      <nav className='cocktail-tabs' aria-label='Cocktail-Navigation'>
        {allCocktails.map((cocktails,keys)=>{
            const isActive=keys===currentIndex;
            
            return(
              <button key={cocktails.id} className={`${isActive? 'text-white border-white': 'text-white/50 border-white/50'}`} onClick={()=>gotoSlide(keys)}>
                {cocktails.name}
              </button>
            )
        })}
